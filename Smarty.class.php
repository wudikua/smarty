 ...
 */
    ...
    function fetch($resource_name, $cache_id = null, $compile_id = null, $display = false)
    {
        ...
        if ($display && !$this->caching && count($this->_plugins['outputfilter']) == 0) {
            ...
            if (!empty($this->recompile_content)) {
                $temp = $this->recompile_content;
                $this->recompile_content = null;
                ob_end_clean();
                return $temp;
            }
   ...
   /**
     ...
     * @param $dependency
     */
    function _extend_recompile($dependency)
    {
        $dependency = unserialize($dependency);
        $template = $dependency['fetch_template'];
        unset($dependency['fetch_template']);
        foreach ($dependency as $tpl=>$time) {
            if (filemtime($this->template_dir . '/' . trim($tpl, '"')) > $time) {
                $this->force_compile = true;
                //由于继承的父模版有修改，所以重新编译
                $this->recompile_content = $this->fetch($template);
            }
        }
    }
