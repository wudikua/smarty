...
class Smarty_Compiler extends Smarty {
     ...
     * @var array
     */
    var $_child_block           =   array();

    /**
     * 解析模版block节点时候用的辅助栈
     * @var array
     */
    var $_child_block_stack     =   array();

    /**
     * 所有继承的父模版 模版名=>时间戳
     * @var array
     */
    var $extend_blocks = array();
     ...
     * @param $content
     */
    function create_block($name, $content)
    {
        return $this->left_delimiter . 'block name=' . $name . $this->right_delimiter . PHP_EOL .
        $content . PHP_EOL . $this->left_delimiter . '/block' . $this->right_delimiter . PHP_EOL;
    }
     ...
     * @return string
     */
    function parse_block($source_content)
    {
        $ldq = preg_quote($this->left_delimiter, '~');
        $rdq = preg_quote($this->right_delimiter, '~');
        $text_blocks = explode(PHP_EOL, $source_content);
        $n = count($text_blocks);
        for ($i=0; $i<$n; $i++) {
            $line = $text_blocks[$i];
            if (preg_match('/'.$ldq.'block\sname=(.+?)\s*?'.$rdq.'/i', $line, $matches)) {
                //block 的开始
                array_push($this->_child_block_stack, array('name'=>$matches[1], 'content'=>''));
            } else if (preg_match('/'.$ldq.'\s*\/block\s*'.$rdq.'/i', $line, $matches)) {
                //block 的结束
                $block = array_pop($this->_child_block_stack);
                if (!empty($this->_child_block_stack)) {
                    $parent_block = array_pop($this->_child_block_stack);
                    $parent_block['content'] .= $this->create_block($block['name'], $block['content']);
                    array_push($this->_child_block_stack, $parent_block);
                } else if (empty($this->_child_block[$matches[1]])){
                    //子层模版中没有重写过改block才解析该block
                    $this->_child_block[trim($block['name'], "'\"")] = $block['content'].PHP_EOL;
                }
            } else {
                //block 的内容
                if (!empty($this->_child_block_stack)) {
                    $block = array_pop($this->_child_block_stack);
                    $block['content'] .= $line;
                    array_push($this->_child_block_stack, $block);
                }
            }
        }
        $this->_child_block_stack = array();
    }
     ...
     * @return string
     */
    function replace_block($source_content)
    {
        $replace_content = '';
        $ldq = preg_quote($this->left_delimiter, '~');
        $rdq = preg_quote($this->right_delimiter, '~');
        ...
        for ($i=0; $i<$n; $i++) {
            ...
            if (preg_match('/'.$ldq.'block\sname=(.+?)\s*?'.$rdq.'/i', $line, $matches)) {
                ...
                unset($matches);
                //查找有没有block before
                if (isset($this->_child_block[$blockName.'.before'])) {
                    //添加block before
                    $replace_content .= $this->create_block($blockName.'.before', $this->_child_block[$blockName.'.before']);
                }
                array_push($this->_child_block_stack, $blockName);
                if (isset($this->_child_block[$blockName])) {
                    //忽略父模版此节点下的所有内容
                    while (in_array($blockName, $this->_child_block_stack) && $i<$n) {
                        $i++;
                        $line = $text_blocks[$i];
                        if (preg_match('/'.$ldq.'block\sname=(.+?)\s*?'.$rdq.'/i', $line, $matches)) {
                            array_push($this->_child_block_stack, $matches[1]);
                        }
                        if (preg_match('/'.$ldq.'\s*\/block\s*'.$rdq.'/i', $line)) {
                            array_pop($this->_child_block_stack);
                        }
                    }
                    //替换节点内容为子模版的内容
                    $replace_content .= $this->create_block($blockName, $this->_child_block[$blockName]);
                    continue;
                }
            } else if (preg_match('/'.$ldq.'\s*\/block\s*'.$rdq.'/i', $line)) {
                //block 的结束
                $blockName = array_pop($this->_child_block_stack);
                //查找有没有block after
                if (isset($this->_child_block[$blockName.'.after'])) {
                    //添加block after
                    $replace_content .= $this->create_block($blockName.'.after', $this->_child_block[$blockName.'.after']);
                }
            }
            $replace_content .= $line . PHP_EOL;
        }
        $this->_child_block_stack = array();
    ...
    function _compile_file($resource_name, $source_content, &$compiled_content)
    {
        ...
        //模版继承逻辑代码
        $find = preg_match('/'.$ldq.'(extend\s(.+?)\s*?)'.$rdq.'/is',$source_content,$matches);
        if ($find) {
            //把extend标签替换为空
            $source_content = str_replace($matches[0],'',$source_content);
            // 记录页面中的block标签
            $this->parse_block($source_content);
            // 读取继承的父模板
            $template_tag = $matches[1];
            // 解析extend命令的参数
            if(! preg_match('~^(?:(' . $this->_num_const_regexp . '|' . $this->_obj_call_regexp . '|' . $this->_var_regexp
                . '|\/?' . $this->_reg_obj_regexp . '|\/?' . $this->_func_regexp . ')(' . $this->_mod_regexp . '*))
                      (?:\s+(.*))?$
                    ~xs', $template_tag, $matches)) {
                $this->_syntax_error("unrecognized tag: $template_tag", E_USER_ERROR, __FILE__, __LINE__);
            }
            $tag_args = isset($matches[3]) ? $matches[3] : null;
            $attrs = $this->_parse_attrs($tag_args);
            //获取父模版内容
            $parent_content = file_get_contents($this->template_dir . '/' . trim($attrs['file'], '"'));
            //将子模版中的block 替换到父模版中
            $parent_content = $this->replace_block($parent_content);
            //记录fetch的template name
            if (empty($this->extend_blocks)) {
                $this->extend_blocks['fetch_template'] = $resource_name;
            }
            //记录父模版的修改时间
            $this->extend_blocks[$attrs['file']] = filemtime($this->template_dir . '/' . trim($attrs['file'], '"'));
            //对父模版进行编译
            return $this->_compile_file($attrs['file'], $parent_content, $compiled_content);
        }
        //        //替换所有block标签
        //        $source_content = preg_replace('/'.$ldq.'[\s\/]*?block.*?'.$rdq.'/',"", $source_content);
        ...
        if (!empty($this->extend_blocks)) {
            //写继承的父模版，检测父模版的修改时间
            $template_header .= '<?php $this->_extend_recompile(\''.serialize($this->extend_blocks).'\'); ?>';
            unset($this->extend_blocks);
            unset($this->_child_block);
            unset($this->_child_block_stack);
        }
